...
var SENSOR = {
  ...
  getTemperature: function() { 
    console.log("Getting the current office temperature!");
    return SENSOR.currentTemperature;
  },
  getHumidity: function() { 
    console.log("Getting the current office humidity!");
...
var options = {
  ...
  clientId: 'OFfice_DHT22_Sensor_Homekit'
};
var client = mqtt.connect(options);
console.log("Temp Office Sensor Connected to MQTT broker");
client.subscribe('office/#');
client.on('message', function(topic, message) {
  // message is Buffer 
  data = JSON.parse(message);
  if (data === null) {return null}
  if(topic == 'office/temperature') {
    SENSOR.currentTemperature = parseFloat(data);
    console.log("Office Temperature: " + SENSOR.currentTemperature);
  } else if (topic == 'office/humidity') {
...
var sensorUUID = uuid.generate('hap-nodejs:accessories:officesensor');

// This is the Accessory that we'll return to HAP-NodeJS that represents our fake lock.
var sensor = exports.accessory = new Accessory('Sensor', sensorUUID);

// Add properties for publishing (in case we're using Core.js and not BridgedCore.js)
sensor.username = "C4:5D:1A:1E:52:BA";
